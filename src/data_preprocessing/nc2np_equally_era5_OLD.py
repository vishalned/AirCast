...
def nc2np(path, variables, years, save_dir, partition, num_shards_per_year):
    ...
    if "pm2p5" in variables:
        # make sure years is not less than 2004 and not greater than 2018
        # NOTE: for 2003, the pm2.5 data starts only from 2nd Jan. This isnt consistent with ERA5 data
        # TODO: Some processing is needed to handle 2003 data, for now we ignore that year.
        assert min(years) >= 2003 and max(years) <= 2018
        ps_pm = glob.glob(os.path.join(path, "pm2p5", f"*.nc"))
        ds_pm = xr.open_mfdataset(ps_pm, combine="by_coords", parallel=True)  # dataset for a single variable
    ...
    for year in tqdm(years):
        ...
        for var in variables:
            ...
            if var == "pm2p5":
                HOURS_PER_YEAR = 1460 # 365-day year with 4 time steps per day - (6 hourly)
                # since pm2p5 data is in one file, we consider all time steps for the year so from 00:00 to 18:00
                ds = ds_pm.sel(time=f"{year}")
            else:
        ...
        for shard_id in range(num_shards_per_year):
            ...
            for k in np_vars.keys():
                if k == "pm2p5":
                    start_id = shard_id * (1460 // num_shards_per_year)
                    end_id = start_id + (1460 // num_shards_per_year)
