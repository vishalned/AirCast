...
class Forecast(IterableDataset):
    def __init__(
        self, dataset: NpyReader, max_predict_range: int = 6, random_lead_time: bool = False, hrs_each_step: int = 1, multi_step_rollout: bool = False, k: int = 1
    ) -> None:
        super().__init__()
        ...
        self.k = k # parameter that indicates the number of rollouts
    ...
    def __iter__(self):
        for data, variables, out_variables in self.dataset:
            ...
            inputs = x[: -self.max_predict_range]  # N, C, H, W , This is done to ensure that the last input has an output. if the max_predict range is too large, we might not have an output for the last input
            if self.random_lead_time:
                # predict_ranges = torch.randint(low=1, high=self.max_predict_range, size=(inputs.shape[0],))
                # we choose a random predict range between 6, 12, 24 for each sample
                import random
                ranges = [6, 12, 24]
                predict_ranges = torch.tensor(random.choices(ranges, k=inputs.shape[0])).to(torch.long)
            else:
            ...
            if not self.multi_step_rollout:
                ...
                # for each sample in the batch, we need a y for each k. i.e we need the y for each rollout
                outputs = []
                for i in range(self.k):
                    output_ids = torch.arange(inputs.shape[0]) + predict_ranges * (i + 1)
                    outputs.append(y[output_ids])
                outputs = torch.stack(outputs, dim=1) # now the rollouts are in dim 1 # N, k, V, H, W
