...
from climax.utils.data_utils import get_region_info, CHEMICAL_VARS
...
class RegionalForecastDataModule(LightningDataModule):
    ...
    def __init__(
        ...
        k: int = 1, # number of rollouts
        batch_size: int = 64,
        num_workers: int = 0,
        pin_memory: bool = False,
        chem_transform: str = 'log', # 'aurora' or 'log' transformations. Aurora is x + log(x)
    ):
        super().__init__()
        ...
        if chem_transform == 'aurora':
            self.transforms = self.get_transformations_aurora()
            self.output_transforms = self.get_normalize(out_variables)
        elif chem_transform == 'log':
            # for a log transform we also have an inverse, hence the output transform will also have the inverse of log
            self.transforms = self.get_transformations_log()
            self.output_transforms = self.get_transformations_log(out_variables)
            self.out_normalize = self.get_normalize(out_variables)
        else:
        ...
        self.chemical_indices = []
        for i, var in enumerate(variables):
            if var.split("_")[0] in CHEMICAL_VARS:
                self.chemical_indices.append(i)


    def get_transformations_aurora(self, variables=None):
        return transforms.Compose(
            [
                self.get_normalize(variables),
                self.get_chem_transform()
            ]
        )
    
    def get_transformations_log(self, variables=None, inverse = False):
        return transforms.Compose(
            [
                self.get_normalize(variables),
                self.get_log_transform()
            ]
        )
    
    def get_log_transform(self):
        def log_transform(inp):
            for i in self.chemical_indices:
                # inp[i] = np.log(torch.max(inp[i], torch.tensor(1e-4)))
                inp[i] = (np.log(torch.max(inp[i], torch.tensor(1e-4))) - np.log(1e-4)) / np.log(1e-4)
            return inp

        return transforms.Lambda(log_transform)

    def get_chem_transform(self):
        # this is a special transformation only for the chemical variables 
        # the formula is for every value in x, we apply the following transformation
        # x_transformed = 0.5 * min(x, 2.5) + 0.5 * (log(max(x, 10^-4)) - log(10^-4)/log(10^-4))
        ...
        return transforms.Lambda(chem_transform)
            ...
            self.data_train = ShuffleIterableDataset(
                IndividualForecastDataIter(
                    Forecast(
                        ...
                        multi_step_rollout=self.hparams.multi_step_rollout,
                        k=self.hparams.k,
                    ),
