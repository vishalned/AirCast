...
class ClimaX(nn.Module):
    ...
    def __init__(
        ...
        for vars in default_vars:
            if vars.split('_')[0] in CHEMICAL_VARS or vars == "tc_no":
                self.chemical_var.append(vars)
        
        self.weather_var = [var for var in default_vars if var not in self.chemical_var]
        ...
        # prediction head
        if len(self.weather_var) > 0:
            self.head_weather = nn.ModuleList()
            for _ in range(decoder_depth):
                self.head_weather.append(nn.Linear(embed_dim, embed_dim))
                self.head_weather.append(nn.GELU())
            self.head_weather.append(nn.Linear(embed_dim, len(self.default_vars) * patch_size**2))
            self.head_weather = nn.Sequential(*self.head_weather)

        # we create a new head for chemical variables. 
        if len(self.chemical_var) > 0:
            self.head_chem = nn.ModuleList()
            for _ in range(decoder_depth):
                self.head_chem.append(nn.Linear(embed_dim, embed_dim))
                self.head_chem.append(nn.GELU())
            self.head_chem.append(nn.Linear(embed_dim, len(self.default_vars) * patch_size**2))
            self.head_chem = nn.Sequential(*self.head_chem)
    ...
    def aggregate_variables(self, x: torch.Tensor):
        ...
        x, _ = self.var_agg(var_query, x, x) # BxL, V, D
